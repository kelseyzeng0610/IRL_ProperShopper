...
if __name__ == "__main__":
    print("Testing TrajectoryRecorder...")

    recorder = TrajectoryRecorder(save_dir="./test_demonstrations")

    # Simulate recording a few episodes
    for ep in range(3):
        dummy_obs = {
            'observation': {
                'players': [{
        ...
        recorder.start_episode(dummy_obs)

        # Simulate some steps
        for step in range(10):
            recorder.record_step(f"NORTH", dummy_obs, timestep=step)

        recorder.end_episode(success=(ep % 2 == 0), final_obs=dummy_obs)
    ...
    recorder2.load(filename)
    print(f"\nSuccessfully loaded {len(recorder2.all_trajectories)} trajectories")
