        ...
        for goalName in jsonifiedQTable:
            ...
            for k, v in jsonifiedQTable[goalName].items():
                if goalName == "INTERACT":
                    ...
                    klist = json.loads(k)
                    keyTuple = tuple(np.array((klist[0], klist[1])))
...
def getForbiddenActions(goal, allActions):
    # when navigating the store, in the interest of training time, just omit the agent from dropping the cart and needlessly interacting
    match goal['type']:
        case "CART_RETURN" | "BASKET_RETURN" | "REGISTER" | "EXIT" | "WALKWAY" | "AISLE" | "SHELF_NAV" | "EAST_WALKWAY" | "COUNTER_NAV" | "LEAVE_COUNTERS":
            options = ["INTERACT", "TOGGLE_CART"]
            return [allActions.index(opt) for opt in options]
...
def getAction(agent, state, current_goal, basketMode, action_commands):
    ...
    forbidden = getForbiddenActions(current_goal, action_commands)
    ...
    if current_goal['type'] == "PICKUP_ITEM":
        ...
        learningMode = True
        action_index = agent.choose_action(state, forbidden)
        action = "0 " + action_commands[action_index]

    return action, action_index, learningMode, forbidden

if __name__ == "__main__":
    # Remove the NOP and RESET actions from action space
...
####################
    ...
    training_time = 10
    episode_length = 3000 # Increase max episode length since sequences can be long
    success = 0
    for i in range(training_time):
        ...
        while not state['gameOver']:
            ...
            if learningMode:
                ...
                # Update Q-table
                agent.learning(action_index, reward, state, next_state, forbiddenActions=forbidden)
            ...
            if state['gameOver'] == True:
                ...
                print("*** Game was set to over in reward function")
                break
